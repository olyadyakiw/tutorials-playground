...
const Item = ({
    ...
    const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        event.stopPropagation()
        if (!id) return
        const promise = create({ title: 'Untitled', parentDocument: id }).then(docunentId => {
            if (!expanded) {
                onExpand?.()
            }
            router.push(`/documents/${docunentId}`)
        })
...
Item.Sketeton = function ItemSkeleton({ level }: { level?: number }) {
    return (
