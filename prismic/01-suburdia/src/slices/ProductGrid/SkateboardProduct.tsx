...
async function getDominatColor(url: string) {
    ...
    return json.dominant_colors.vibrant?.hex || json.dominant_colors_vibrant_light?.hex
}
...
export default async function SkateboardProduct({ id }: Props) {
    ...
    const dominantColor = isFilled.image(product.data.image) ? await getDominatColor(product.data.image.url) : undefined

    return (
        <div className="group relative mx-auto w-full max-w-72 px-8 pt-4">
            <VerticalLine className={clsx(VERICAL_LINE_CLASSES, 'left-4')} />
            <VerticalLine className={clsx(VERICAL_LINE_CLASSES, 'right-4')} />
